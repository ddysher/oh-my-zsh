...
if [[ `hostname` == "DeyuanCloud" ]] || [[ `hostname` == "ubuntu-vm" ]]; then
   PROMPT='%{$fg_bold[blue]%}$USER %{$reset_color%}in %{$fg_bold[yellow]%}%m %{$reset_color%}at %{$fg_bold[green]%}{${PWD/#$HOME/~}%u} $(git_prompt_info)% %{$fg_bold[magenta]%} ○ %{$RESET_COLOR%} '
else
   PROMPT='%{$fg_bold[blue]%}$USER %{$reset_color%}in %{$fg_bold[yellow]%}%m %{$reset_color%}at %{$fg_bold[green]%}{{${PWD/#$HOME/~}%u}} $(git_prompt_info)% %{$fg_bold[magenta]%}
○ %{$RESET_COLOR%} '
fi
